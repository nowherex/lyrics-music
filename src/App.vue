<template>
  <div class="relative min-h-screen md:flex bg-primary">
    <!-- mobile menu bar -->
    <div class="bg-selected text-white flex justify-between items-center md:hidden">
      <div>
      ...
      <!-- mobile menu button -->
      <button @click="showMobileMenu" :class="`${mobileMenuActive ? 'bg-selected-light' : ''} p-4 focus:outline-none`">
        <i class="fas fa-bars"></i>
      </button>
    </div>

      <!-- sidebar -->
    <div :class="`${mobileMenuActive ? '' : '-translate-x-full'} bg-primary text-white w-64 space-y-6 items-center py-6 absolute inset-y-0 left-0 transform md:relative md:translate-x-0 transition duration-200 ease-in-out`">
      ...
      <nav class="pl-10">
        <button v-for="(tab, index) in tabs" :key="index" @click="pageId = tab.id" :class="`focus:outline-none flex items-center py-2 px-4 hover:bg-secondary transition duration-200 rounded-l-lg w-full ${ pageId === tab.id ? 'text-white bg-secondary' : 'text-selected-light'}`">
